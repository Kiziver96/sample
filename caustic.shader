...
Shader "caustic"
{
	Properties
	{
		...
		[ASEBegin]_CausticColor("Caustic Color", Color) = (0.8962264,0,0.5303139,0)
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float4 temp_output_6_0 = mul( unity_CameraToWorld, appendResult49_g1 );
				float3 worldToObj11 = mul( GetWorldToObjectMatrix(), float4( temp_output_6_0.xyz, 1 ) ).xyz;
				float temp_output_8_0 = saturate( ( 1.0 - ( length( worldToObj11 ) * 2 ) ) );
				float time21 = _TimeParameters.x;
				float2 voronoiSmoothId21 = 0;
				float3 worldToObjDir16 = mul( GetWorldToObjectMatrix(), float4( temp_output_6_0.xyz, 0 ) ).xyz;
				float3 ase_objectScale = float3( length( GetObjectToWorldMatrix()[ 0 ].xyz ), length( GetObjectToWorldMatrix()[ 1 ].xyz ), length( GetObjectToWorldMatrix()[ 2 ].xyz ) );
				float2 coords21 = (( worldToObjDir16 * ase_objectScale )).xz * _Scale;
				float2 id21 = 0;
				float2 uv21 = 0;
				float voroi21 = voronoi21( coords21, time21, id21, uv21, 0, voronoiSmoothId21 );
				
				float4 appendResult25 = (float4(0.0 , 0.0 , 0.0 , temp_output_8_0));
				
				float3 BakedAlbedo = 0;
				float3 BakedEmission = 0;
				float3 Color = ( _CausticColor * temp_output_8_0 * voroi21 ).rgb;
				float Alpha = appendResult25.x;
